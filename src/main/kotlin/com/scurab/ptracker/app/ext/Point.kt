...
fun Point.normalize(size: Size): Point {
    return if (size.isEmpty()) {
        ...
        Point((x / abs(size.width)).coerceIn(NormalizedRange), (1f - (y / abs(size.height))).coerceIn(NormalizedRange))
    }
}
...
fun Point.normalize(rect: Rect): Point {
    return if (rect.width == 0f || rect.height == 0f) {
        ...
        Point(((x - rect.left) / abs(rect.width)).coerceIn(NormalizedRange), ((y - rect.top) / abs(rect.height)).coerceIn(NormalizedRange))
    }
}
